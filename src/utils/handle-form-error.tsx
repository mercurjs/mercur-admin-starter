import { DeepMap, FieldError, FieldValues, SubmitErrorHandler } from 'react-hook-form';
...
export const handleFormError: SubmitErrorHandler<FieldValues> = errors => {
  ...
  Controller.custom(t => <FormErrorToaster toast={t} message={list} title={title} />, {
    position: 'top-right',
    duration: 3000,
    ariaProps: {
      role: 'alert',
...
function getFormErrors(errors: DeepMap<FieldValues, FieldError>) {
  const messages: string[] = Object.values(errors).reduce((acc, { message }) => {
    if (message) {
      acc.push(message);
    }
  ...
  const title = messages.length > 1 ? `There were ${messages.length} errors with your submission` : 'There was an error with your submission';
