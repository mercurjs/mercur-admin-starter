...
const Overview = () => {
  ...
  const { state: stateCustom, close: closeCustom, open: openCustom } = useToggleState();

  const { state: stateNew, close: closeNew, open: openNew } = useToggleState();

  const { state: stateDelete, close: closeDelete, open: openDelete } = useToggleState();
  ...
  const onUpdate = () => {
    ...
    updateGiftCard.mutate(
      { status },
      {
        onSuccess: () => notification(t('gift-cards-success', 'Success'), t('gift-cards-successfully-updated-gift-card', 'Successfully updated Gift Card'), 'success'),
        onError: err => notification(t('gift-cards-error', 'Error'), getErrorMessage(err), 'error'),
      },
    );
  };
  ...
  return (
    <>
      <div className="flex flex-col">
        <PageDescription title={t('gift-cards-gift-cards', 'Gift Cards')} subtitle={t('gift-cards-manage', 'Manage the Gift Cards of your Medusa store')} />
        {!isLoading ? (
          <div className="gap-y-xsmall flex flex-col">
            {giftCardWithCurrency ? (
              <GiftCardBanner {...giftCardWithCurrency} onDelete={openDelete} onEdit={() => navigate('/a/gift-cards/manage')} onUnpublish={onUpdate} />
            ) : (
              <BannerCard title={t('gift-cards-are-you-ready-to-sell-your-first-gift-card', 'Are you ready to sell your first Gift Card?')}>
