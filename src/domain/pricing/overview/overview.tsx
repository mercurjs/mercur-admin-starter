...
import { EllipseGreenSolid, EllipseGreySolid, EllipsisHorizontal, ExclamationCircle, PencilSquare, Spinner, Trash } from '@medusajs/icons';
import { DateComparisonOperator, PriceList } from '@medusajs/medusa';
import { clx, Container, DropdownMenu, Heading, IconButton, Input, StatusBadge, Table, Text, usePrompt } from '@medusajs/ui';
import { createColumnHelper, flexRender, getCoreRowModel, getPaginationRowModel, Row, useReactTable } from '@tanstack/react-table';
import { useAdminDeletePriceList, useAdminPriceLists, useAdminUpdatePriceList } from 'medusa-react';
...
const PriceListTableFilters = () => {
  ...
  const onDateChange = (key: 'updated_at' | 'created_at', value?: DateComparisonOperator) => {
    const current = new URLSearchParams(searchParams);
  ...
  return (
    <FilterMenu onClearFilters={onClearFilters}>
      <FilterMenu.Content>
        ...
        <FilterMenu.DateItem name="Created at" value={getDateComparisonOperatorFromSearchParams('created_at', searchParams)} onChange={dc => onDateChange('created_at', dc)} />
        <FilterMenu.Seperator />
        <FilterMenu.DateItem name="Updated at" value={getDateComparisonOperatorFromSearchParams('updated_at', searchParams)} onChange={dc => onDateChange('updated_at', dc)} />
      </FilterMenu.Content>
    </FilterMenu>
  );
};
...
const PriceListOverview = () => {
  ...
  const { price_lists, count, isLoading, isError } = useAdminPriceLists(
    {
      limit: PAGE_SIZE,
      status: getStringArrayFromSearchParams('status', searchParams) as PriceListStatus[] | undefined,
      created_at: getDateComparisonOperatorFromSearchParams('created_at', searchParams),
      updated_at: getDateComparisonOperatorFromSearchParams('updated_at', searchParams),
    ...
    return (
      ...
      >
        <div className="flex items-center gap-x-2">
          ...
          <Text className="text-ui-fg-subtle">An error occurred while loading the price lists. Try to reload the page or try again later.</Text>
        </div>
      </Container>
    );
  }
    ...
    <div>
      <div className="flex flex-col gap-y-2">
        <Container className="overflow-hidden p-0">
          <div className="flex flex-col gap-base small:flex-row items-start small:items-center justify-between px-8 pt-6 pb-4">
            <Heading>Price Lists</Heading>
            <div className="grid grid-cols-1 gap-base small:flex small:items-center gap-x-2">
            ...
            <Table>
              <Table.Header>
                {table.getHeaderGroups().map(headerGroup => {
                  return (
                    <Table.Row key={headerGroup.id} className="[&_th]:w-1/5 [&_th:last-of-type]:w-[1%]">
                      {headerGroup.headers.map(header => {
                        return <Table.HeaderCell key={header.id}>{flexRender(header.column.columnDef.header, header.getContext())}</Table.HeaderCell>;
                      })}
                    </Table.Row>
                  );
                })}
...
const columns = [
  ...
  columnHelper.accessor('description', {
    header: 'Description',
    cell: info => <span className="overflow-hidden text-ellipsis whitespace-nowrap">{info.getValue()}</span>,
  }),
  columnHelper.accessor('status', {
    ...
    cell: ({ getValue, row }) => {
      ...
      const color = isExpired ? 'red' : isDraft ? 'grey' : isScheduled ? 'orange' : 'green';

      const text = isExpired ? 'Expired' : isDraft ? 'Draft' : isScheduled ? 'Scheduled' : 'Active';
...
const PriceListTableRowActions = ({ row }: PriceListTableRowActionsProps) => {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    return deleteFn(undefined, {
      onSuccess: () => {
        notification('Price list deleted', `Successfully deleted ${row.original.name}`, 'success');
      },
      onError: err => {
  ...
  const toggleStatus = async (e: React.MouseEvent) => {
    ...
    return updateFn(
      {
        status: row.original.status === 'active' ? PriceListStatus.DRAFT : PriceListStatus.ACTIVE,
      },
      {
        onSuccess: () => {
          notification('Price list updated', `Successfully updated ${row.original.name}`, 'success');
        },
        onError: err => {
          notification('An error occurred', getErrorMessage(err), 'error');
        },
  ...
  const isExpired = row.original.ends_at ? new Date(row.original.ends_at) < new Date() : false;
      ...
      <DropdownMenu.Content>
        ...
        {!isExpired && (
          <DropdownMenu.Item onClick={toggleStatus}>
            ...
            <span className="ml-2">{isActive ? 'Mark as draft' : 'Mark as active'}</span>
          </DropdownMenu.Item>
        )}
