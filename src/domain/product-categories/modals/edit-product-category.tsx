...
import MetadataForm, { getSubmittableMetadata } from '../../../components/forms/general/metadata-form';
 ...
 */
function EditProductCategoriesSideModal(props: EditProductCategoriesSideModalProps) {
  const { isVisible, close, activeCategory, categories } = props;

  const { t } = useTranslation();
  const notification = useNotification();

  const { mutateAsync: updateCategory } = useAdminUpdateProductCategory(activeCategory?.id);
  ...
  const onSave = async (data: CategoryFormData) => {
    try {
      ...
      notification(t('modals-success', 'Success'), t('modals-successfully-updated-the-category', 'Successfully updated the category'), 'success');
      close();
    } catch (e) {
      ...
      <div className="flex h-full flex-col justify-between overflow-auto">
        ...
        <div className="flex items-center justify-between p-6">
          ...
          <div className="gap-x-small flex">
            <Button size="small" variant="primary" disabled={!isDirty || !isValid || isSubmitting} onClick={handleSubmit(onSave)} className="rounded-rounded">
              {t('modals-save-category', 'Save category')}
            </Button>
          </div>
        </div>
        <h3 className="inter-large-semibold flex items-center gap-2 text-xl text-gray-900 px-6">{t('modals-edit-product-category', 'Edit product category')}</h3>
        {/* === DIVIDER === */}
        ...
        <div className="flex-grow px-6">
          <InputField
            ...
            type="string"
            className="my-6"
            placeholder={t('modals-give-this-category-a-name', 'Give this category a name') as string}
          ...
          <TextArea
            ...
            className="my-6"
            placeholder={t('modals-give-this-category-a-description', 'Give this category a description') as string}
