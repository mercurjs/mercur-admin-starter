...
const ProductOverview = ({
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 small:grid-cols-4">
...
const ProductTile = ({ product }) => {
  ...
  return (
    <div className="p-base rounded-rounded w-fit hover:bg-grey-5 group flex-col">
      <div className="relative">
        ...
        <Link to={`${product.id}`}>
          {product.thumbnail ? (
            <img
              className="rounded-rounded block max-w-[200px] object-cover"
              src={product.thumbnail}
            />
          ) : (
            <div className="bg-grey-5 rounded-rounded flex w-[200px] h-[134px] items-center justify-center">
              <ImagePlaceholder />
            </div>
          )}
          <div>
