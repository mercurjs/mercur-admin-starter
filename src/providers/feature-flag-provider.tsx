...
export const FeatureFlagProvider = ({ children }: PropsWithChildren) => {
  ...
  const featureToggleList = featureFlags.reduce((acc, flag) => ({ ...acc, [flag.key]: flag.value }), {} as Record<string, boolean>);

  const isFeatureEnabled = (flag: string) => !!featureToggleList[flag];

  return <FeatureFlagContext.Provider value={{ isFeatureEnabled, featureToggleList }}>{children}</FeatureFlagContext.Provider>;
};
