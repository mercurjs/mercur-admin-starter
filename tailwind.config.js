...
try {
  uiPath = path.resolve(path.dirname(require.resolve('@medusajs/ui')), '../..', '**/*.{js,ts,jsx,tsx}');
} catch (e) {
  // ignore
}
...
module.exports = {
  presets: [preset],
  darkMode: 'class',
  content: [path.join(__dirname, 'src/**/*.{js,ts,jsx,tsx}'), path.join(__dirname, './index.html'), uiPath].concat(extensionPaths.map(ext => path.join(__dirname, ext))),
  theme: {
    screens: {},
    extend: {
      transitionProperty: {
